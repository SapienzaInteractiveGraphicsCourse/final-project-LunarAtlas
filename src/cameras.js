...
export function positionCamera(camera, subject, position) {
  ...
  return { subject, ...cartesianToLatLon(position), distance: position.length() };
}

export function updateLatLon(camera, camera_position) {
  camera.getWorldPosition(_cameraWorldPos);
  camera_position.subject.getWorldPosition(_subjectWorldPos);

  _offset.subVectors(_cameraWorldPos, _subjectWorldPos);
  camera_position.distance = _offset.length();
  _offset.normalize();
  camera_position.lat = Math.asin(Math.max(-1, Math.min(1, _offset.y)));
  camera_position.lon = Math.atan2(_offset.x, _offset.z);
